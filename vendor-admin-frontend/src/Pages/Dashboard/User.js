...
export const User = () => {
  ...
  const fetchVendors = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("https://super-admin-ga55.onrender.com/api/auth/getAllVendors", {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeactivate = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`https://super-admin-ga55.onrender.com/api/auth/vendors/${id}/deactivate`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleRequestReactivation = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`https://super-admin-ga55.onrender.com/api/auth/vendors/${id}/activate`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
